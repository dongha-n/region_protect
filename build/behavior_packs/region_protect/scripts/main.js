...
function generate_outline_boundary({ x, y, z }) {
    return {
        lbb: {
            x: x - R,
            y: y - R,
...
world.afterEvents.playerPlaceBlock.subscribe((param) => {
    ...
    if (param.block.typeId === REGION_PROTECT_BLCOK) {
        ...
        storage[param.player.id] = param.block.location;
        ...
        generate_outline({ ...param.block.location, y: param.block.location.y + R }).map(({ begin, end, block }) => param.dimension.fillBlocks(begin, end, block));
    }
});
...
//# sourceMappingURL=../../_region_protectDebug/main.js.map
